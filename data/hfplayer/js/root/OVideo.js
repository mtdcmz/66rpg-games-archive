...
function OVideo(){
    ...
    var videomaskBG = document.getElementById("videomaskBG");
    ...
    this.drawLoadingVideo = function(){
        videoText.style.fontSize=parseInt(23/zoom)+"px";
        videoText.style.color="#777777";
        videoText.style.fontFamily="微软雅黑";
        videoText.innerHTML = loadTexts;
        self.resizeVideoMask(videoText);
        self.resizeVideoMask(videomaskBG);
    }
    this.init = function(){
        type = getC();
       // alert(type);
    ...
    this.VedioManager={
        ...
        play:function(args){
            ...
            if(parseInt(args[7]) == 1){
                self._starttime = tv.system.vars.getVar(parseInt(args[8]))/1000;
            }else{
                self._starttime = parseInt(args[8])/1000;
            }
            self._endtime =  parseInt(args[9])/1000;

            self._src = fileListFato(("graphics/"+args[0]).toLowerCase(),"videoPlay");
            self._video = document.getElementById("videos");
            var blob = oVideoArr[self._src];
            // if(!blob){
            //     return;
            // }
            //设置scoure；
            ...
            if(!self.isClear){
                switch(parseInt(args[0]))
                {
                    ...
                    case 5:
                        ...
                        tv.system.vars.setVar(timer[1],curTimer);
                        break;
                }
            }else{
                self.isFinish = true;
    ...
    this.videoDomDate = function(args){
        //xy
        if(parseInt(args[1]) == 1){
            self._x = tv.system.vars.getVar(parseInt(args[2]))/zoom;
            self._y = tv.system.vars.getVar(parseInt(args[3]))/zoom;
        }else{
        ...
        if(parseInt(args[4]) == 1) {
            self._width = tv.system.vars.getVar(parseInt(args[5]))/zoom;
            self._height = tv.system.vars.getVar(parseInt(args[6]))/zoom;
        } else {
    ...
    this.drawCanvasDate = function(args){
        //xy
        if(parseInt(args[1]) == 1){
            self._x = tv.system.vars.getVar(parseInt(args[2]));
            self._y = tv.system.vars.getVar(parseInt(args[3]));
        }else{
        ...
        //wh
        if(parseInt(args[4]) == 1) {
            self._width = tv.system.vars.getVar(parseInt(args[5]));
            self._height = tv.system.vars.getVar(parseInt(args[6]));
        } else {
    ...
    this.resizeVideo = function(div){
        if(isPhone){
            if(isVertical){
                ...
                div.style.top = gGameWidth/zoom-this._x+gameCanvas.offsetTop+"px";
                div.style.left = this._y+gameCanvas.offsetLeft+'px';
            }else{
                //横屏
                this.drawVideo(div);
               ...
               div.style.top = this._x+gameCanvas.offsetTop+"px";
               div.style.left = gGameHeight/zoom-this._y+gameCanvas.offsetLeft+'px';
            }
        } else {
            this.drawVideo(div);
           ...
           div.style.top = this._y+gameCanvas.offsetTop+"px";
           div.style.left = this._x+gameCanvas.offsetLeft+'px';
        }
    };
    //旋转視頻前遮罩
    ...
    this.update = function(){
        ...
        if(videomaskBG.style.display =="block"&&videoText.style.display =="block"){
            self.resizeVideoMask(videomaskBG);
            self.resizeVideoMask(videoText);
        }
        //视频移动
        ...
        if(this._video){
            if(gIsApple && mark != "ios"){
                ...
                //androd
                if(this._visible){
                    self._video.style.zIndex = 22;
                    self._video.style.opacity = 1;
                }else{
                    self._video.style.zIndex = -1;
                    self._video.style.opacity = 0;
                }
            }
...
OVideo.prototype = {
    ...
    set move(val){
        ...
        if(gIsApple && mark != "ios"){
            //ios
            if(val[0] == 1){
                this._moveendx = tv.system.vars.getVar(parseInt(val[1]));
                this._moveendy = tv.system.vars.getVar(parseInt(val[2]));
            }else{
                this._moveendx = val[1];
                this._moveendy = val[2];
            }
            if(val[3] == 1){
                this._moveendw = tv.system.vars.getVar(parseInt(val[4]));
                this._moveendh = tv.system.vars.getVar(parseInt(val[5]));
            }else{
            ...
            //androd
            if(val[0] == 1){
                this._moveendx = tv.system.vars.getVar(parseInt(val[1]))/zoom;
                this._moveendy = tv.system.vars.getVar(parseInt(val[2]))/zoom;
            }else{
                this._moveendx = val[1]/zoom;
                this._moveendy = val[2]/zoom;
            }
            if(val[3] == 1){
                this._moveendw = tv.system.vars.getVar(parseInt(val[4]))/zoom;
                this._moveendh = tv.system.vars.getVar(parseInt(val[5]))/zoom;
            }else{
